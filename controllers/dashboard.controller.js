...
module.exports.editUser = async (req, res) => {
    ...
    uploader(req, res, async (error) => {
        ...
        let errorMessage = "";
        let avatarPath =
            avatar && `public/uploads/${v4UniqueId()}-${avatar.originalname}`;
        ...
        // Re-path to store in db
        avatarPath = avatarPath && avatarPath.split("public")[1];

        // Check if image too large
        if (error) {
            errorMessage = "Image is larger than 10MB";
        }

        // Check empty workplace
        if (permission === null) {
            errorMessage = "Permission can not be empty";
        }
        ...
        if (errorMessage) {
            ...
            // Convert permission input into valid type of collection's field
            let tempPermission = permission && permission.split(",");
            let permissionObj =
                tempPermission &&
...
module.exports.addNewComment = async (req, res) => {
    ...
    req.body.commentUniqueId = commentUniqueId;
